...
export default function MultiplayerQuizHost({ quiz, user }: MultiplayerQuizHostProps) {
     ...
     const updateState = (overrides: Partial<IMultiplayerHostState>) => setState(prev => ({ ...prev, ...overrides }));
     ...
     useEffect(() => {
          ...
          socket.on("end round", (players: IQuizUser[]) => {
               ...
               leaderboardTimer.current = setTimeout(() =>
                    updateState({ phase: "leaderboard" }), 4000);
          })
     ...
     const handleChangeTime = (time: number) => {
          ...
          if (time <= 0) socket.emit("phase change", gameCode, "question");
     }
          ...
          <>
               <QuizWrapper quizDetails={{ name, teacher, subject, createdAt }}>
                    ...
                    {(isStarted && phase === "countdown") && (
                         <Timer time={startTimer} initialTime={QUIZ_START_TIME} onTimeChange={handleChangeTime} />
                    )}
                    ...
                    {(isStarted && phase === "leaderboard") && (
                         <div className="flex items-center justify-center flex-col gap-2">
                              ...
                              <Button variant="outline" onClick={() => currIdx !== questions.length - 1 ? nextQuestion() : finishGame()}>
