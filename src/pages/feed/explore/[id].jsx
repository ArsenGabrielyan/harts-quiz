...
export default function SingleQuiz({session,teacherId,id}){
     const router = useRouter();
     const {data: quiz, mutate: updateQuiz, isLoading, isValidating} = useSWR(`/api/questions?id=${id}`,fetcher);
     const {data: currUser, mutate: updateUser} = useSWR(`/api/users?email=${session?.user?.email}`,fetcher)
     const duplicateQuiz = async()=>{
          const res = await toast.promise(axios.post("/api/questions",{reqType: 'duplicate',quizId: quiz?.id}),{
     ...
     return <FeedLayout>
          {isLoading ? <Loader /> : <section className="quiz-info">
               <div className="header">
                    <h1>{quiz?.name}{isValidating && <ThreeDots width={32} height={32}  color="#222"/>}</h1>
...
export const getServerSideProps = async(ctx) => {
     ...
     return quiz ? {props: {session,teacherId: teacher.userId,id}} : {notFound: true}
}